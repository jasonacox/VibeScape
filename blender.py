...
class SeasonBlender:
    ...
    def _build_interpolation_table(self):
        ...
        logger.debug(
            "Built interpolation table with %d key dates", len(self.weight_table)
        )
        ...
        if target_date is None:
            ...
            if date_override:
                try:
                    ...
                    logger.info(
                        "Using DATE override: %s (day %d)",
                        target_date,
                        target_date.timetuple().tm_yday,
                    )
                except (ValueError, TypeError) as e:
                    logger.warning(
                        "Invalid DATE override '%s': %s - using current date",
                        date_override,
                        e,
                    )
    ...
    def get_prompt(self, target_date: date = None) -> Tuple[str, str]:
        ...
        """
        # Determine the date and extract month for context
        if target_date is None:
            date_override = os.environ.get("DATE")
            if date_override:
                try:
        ...
        prompt = season.get_prompt(month=current_month)

        logger.debug(
            "Generated prompt from %s (month %d): %s...",
            season_name,
            current_month,
            prompt[:100],
        )
