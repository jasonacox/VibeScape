...
class TestStylePrefixes:
    ...
    def test_default_style_is_photorealistic(self):
        ...
        assert any(
            word in style for word in ["photorealistic", "cinematic", "detailed"]
        )
...
class TestGetPrompt:
    ...
    def test_prompt_contains_style(self):
        ...
        assert (
            has_default or has_alternate
        ), f"No style prefix found in prompt: {prompt}"

    def test_default_style_used_most_often(self):
        """Test that default style is used most often (~80%) with alternates at 20%."""
        season = ConcreteSeasonForTesting()
        ...
        # Default should be used around 80% of the time (20% alternate)
        # Allow some statistical variance (70-90%)
        assert (
            70 <= default_count <= 90
    ...
    def test_extras_selection_respects_count(self):
        """Test that 1-3 extras are selected with variation."""
        season = ConcreteSeasonForTesting()
        ...
        # Should have 1, 2, or 3 extras (new variation system)
        assert 1 <= extras_in_prompt <= 3
...
class TestPromptVariety:
    ...
    def test_all_scenes_can_be_selected(self):
        ...
        assert len(scenes_seen) == len(
            season.scene_keywords
    ...
    def test_all_extras_can_be_selected(self):
        ...
        assert (
            len(extras_seen) >= len(season.extras) - 1
    ...
    def test_all_alternate_styles_can_be_used(self):
        ...
        assert len(styles_seen) >= 2, f"Only saw {len(styles_seen)} alternate styles"
...
class TestEdgeCases:
    ...
    def test_many_extras(self):
        ...
        # Should select 1-3 extras (new variation system)
        extra_count = sum(1 for i in range(100) if f"extra{i:03d}" in prompt)
        assert 1 <= extra_count <= 3, f"Expected 1-3 extras, found {extra_count}"
