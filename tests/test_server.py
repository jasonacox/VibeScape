...
class TestGenerateScene:
    ...
    async def test_generate_scene_builds_prompt_when_none(self):
        ...
        with patch.object(server, '_generate_swarmui', new_callable=AsyncMock) as mock_gen:
    ...
    async def test_generate_scene_uses_provided_prompt(self):
        ...
        with patch.object(server, '_generate_swarmui', new_callable=AsyncMock) as mock_gen:
            ...
            result = await server.generate_scene(prompt="custom prompt", season_name="test")
    ...
    async def test_generate_scene_routes_to_swarmui(self):
        ...
        with patch('server.IMAGE_PROVIDER', 'swarmui'):
            with patch('server._generate_swarmui', new_callable=AsyncMock) as mock_swarm:
    ...
    async def test_generate_scene_routes_to_openai(self):
        ...
        with patch('server.IMAGE_PROVIDER', 'openai'):
            with patch('server._generate_openai', new_callable=AsyncMock) as mock_openai:
...
class TestImageStatusEndpoint:
    ...
    def test_image_status_available_false_when_no_cache(self, test_client, reset_server_globals):
...
class TestBackgroundTasks:
    ...
    @pytest.mark.asyncio
    async def test_background_generate_updates_cache(self, sample_base64_image, reset_server_globals):
