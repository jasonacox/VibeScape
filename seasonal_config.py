...
SEASONAL_WEIGHTS = {
    ...
    (11, 26): {
        "thanksgiving": 0.80,
        "christmas": 0.20,
    },  # Day after Thanksgiving - Christmas begins
...
def validate_config():
    ...
    for date_key, weights in SEASONAL_WEIGHTS.items():
        ...
        if abs(total - 1.0) > 0.01:  # Allow small floating point errors
            print(
                f"WARNING: Weights for {date_key[0]}/{date_key[1]} sum to {total:.2f}, not 1.0"
            )
...
if __name__ == "__main__":
    ...
    for date_key in [(4, 20), (7, 4), (10, 31), (11, 25), (12, 25), (1, 1)]:
        if date_key in SEASONAL_WEIGHTS:
            ...
            weight_str = ", ".join(
                [f"{k}: {v*100:.0f}%" for k, v in sorted(weights.items())]
            )
